...
plots[0].set_title("Traffic per Replica", pad=0)
plots[0].set_ylabel("Bytes per App Req.", labelpad=1)
plots[0].yaxis.set_major_locator(MultipleLocator(100))
plots[0].yaxis.set_minor_locator(MultipleLocator(50))
plots[0].set_ylim(0, 250)
# plots[0].set_yscale("log")
plots[0].yaxis.set_major_formatter(k_formatter)
plots[1].set_title("Communication per Repl.", pad=0)
...
plots[1].yaxis.set_minor_locator(MultipleLocator(1))
plots[1].set_ylim(0, 13)
for plot in plots:
...
for i, experiment in enumerate(ALGORITHMS):
    ...
    for num_replicas in range(3, 31 + 2, 2):
        ...
        ys_bytes.append(total_bytes / total_requests / num_replicas)
        ys_msgs.append(total_msgs / total_requests / num_replicas)
